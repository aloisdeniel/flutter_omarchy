...
///
/// The [OmarchyTextInput] widget provides a styled text field with support for
...
class OmarchyTextInput extends StatefulWidget {
  /// Creates a text input widget with customizable properties.
  const OmarchyTextInput({
    super.key,
    this.initialValue,
    this.placeholder,
    this.controller,
  ...
  @override
  State<OmarchyTextInput> createState() => OmarchyTextInputState();
}

class OmarchyTextInputState extends State<OmarchyTextInput>
  ...
  Widget build(BuildContext context) {
    ...
    final effectiveTextStyle = theme.text.normal
        .copyWith(color: theme.colors.foreground)
        .merge(widget.style);

    final effectiveCursorColor =
        widget.cursorColor ?? theme.colors.selectedText;

    final effectivePadding =
        widget.padding ??
        DefaultPadding.maybeOf(context) ??
        const EdgeInsets.symmetric(horizontal: 0, vertical: 8);

    final effectiveInputPadding = widget.inputPadding ?? EdgeInsets.zero;

    final effectivePlaceholderStyle = theme.text.normal
        .copyWith(color: theme.colors.bright.black)
        .merge(widget.placeholderStyle);

    final effectivePlaceholderAlignment =
        widget.placeholderAlignment ?? Alignment.topLeft;
    ...
    final effectiveScrollbarPadding =
        widget.scrollbarPadding ?? EdgeInsets.zero;
...
class _InputSelectionGestureDetectorBuilder
  ...
  _InputSelectionGestureDetectorBuilder({required OmarchyTextInputState state})
    : _state = state,
      super(delegate: state);

  final OmarchyTextInputState _state;
